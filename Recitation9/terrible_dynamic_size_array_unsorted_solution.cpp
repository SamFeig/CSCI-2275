...
bool contains(const int_array& arr, const int& target)
{
	...
	for (i = 0; i < arr.count; ++i); // here is a semicolon
	{
...
void resize(int_array& arr)
{
	...
	arr.data = new_data;
	delete [] arr.data; // delete after update
...
void add(int_array& arr, const int& payload)
{
	...
	arr.data[++arr.count] = payload; // array indexing issue. because of preincrement one position will be missed
...
{
	unsigned int i = 0;
	if (arr.count = 0) // comparison operator vs assignment operator
